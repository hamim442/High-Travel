...
export default function MainTravelPlan() {
    ...
    const totalPrice =
        trains.reduce((total, train) => total + train.price, 0) +
        cars.reduce((total, car) => total + car.price, 0) +
        flights.reduce((total, flight) => total + flight.price, 0) +
        accommodations.reduce(
            (total, accommodation) => total + accommodation.total_price,
            0
        )

    // You should use the useAuthService hook, you don't need to do this in this component.
    useEffect(() => {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            setError(null)
            try {
                // When you are fetching multiple things like this, it might be better to create
                // a bespoke endpoint which returns all of this data at once, doing multiple fetches
                // can be slower. Also if you needed to do multiple fetches you should be using Promise.all()
                ...
                const filteredTrains = trainsData.filter(
                    (train) => train.trip_id === parseInt(tripId)
                )
                ...
                const filteredCars = carsData.filter(
                    (car) => car.trip_id === parseInt(tripId)
                )
                ...
                const filteredFlights = flightsData.filter(
                    (flight) => flight.trip_id === parseInt(tripId)
                )
                ...
                const filteredAccommodations = accommodationsData.filter(
                    (accommodation) =>
                        accommodation.trip_id === parseInt(tripId)
                )
    ...
    return (
        <div className="container">
            <div className="mt-4">
                <h2>
                    {user.username}'s trip to {city.name}
                </h2>
                        ...
                        >
                            {city?.picture_url && (
                                <img
                                    ...
                                    // You should probably put the styles here in a CSS file
                                    style={{
                                        width: '100%',
                                        height: '100%',
                                        objectFit: 'cover',
                    ...
                    <div className="card">
                        <div className="card-body">
                            ...
                            <p className="card-text">
                                Details about food/places/others
                            </p>
                        </div>
